    ...
    .map(
      (r, i) => {
        ...
        function fmtDate(iso) {
          if (!iso) return '-';
          try { const d = new Date(iso); return d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ' ' + d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }); } catch(e) { return iso; }
        }
        const createdBy = r.created_by || '-';
        const updatedInfo = r.updated ? `${r.updated_by || 'unknown'}<br><span style="color:#8899a6;font-size:11px;">${fmtDate(r.updated)}</span>` : '-';
...
<body>
  ...
  <table>
    <thead>
      <tr>
        <th>#</th><th>Category</th><th>Name</th><th>UID</th><th>Status</th><th>Load Time</th><th>Created By</th><th>Last Updated</th><th>Error</th><th>Grafana</th><th>Screenshot</th>
      </tr>
    </thead>
